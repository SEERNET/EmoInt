...
if __name__ == '__main__':
    ...
    with open(os.path.join(pred, 'nb_words.json'), 'r') as f_obj:
        nb_words = json.load(f_obj)['nb_words']
    word_embedding_matrix = np.load(open(os.path.join(pred, 'word_embedding_matrix.npy'), 'rb'))

    train_r_32, train_q_32, train_r_64, train_q_64, train_r_128, train_q_128, \
    train_y, train_lex_r, train_lex_q, train_depths = pickle.load(open(os.path.join(pred, 'train_data.pkl'), 'rb'))

    dev_r_32, dev_q_32, dev_r_64, dev_q_64, dev_r_128, dev_q_128, \
    dev_y, dev_lex_r, dev_lex_q, dev_depths = pickle.load(open(os.path.join(pred, 'dev_data.pkl'), 'rb'))

    test_r_32, test_q_32, test_r_64, test_q_64, test_r_128, test_q_128, \
    test_y, test_lex_r, test_lex_q, test_depths = pickle.load(open(os.path.join(pred, 'test_data.pkl'), 'rb'))
    ...
    cb = [
        ModelCheckpoint(
            os.path.join(log_dir, 'weights_{epoch:02d}_{val_loss:.3f}_{val_acc:.3f}.h5'),
    ...
    if lex:
       ...
       #  met = mdl.evaluate(x=[test_rq, test_qr, test_lex_rq, test_lex_qr], verbose=0,
        ...
        met = mdl.evaluate(x=[test_lex_rq, test_lex_qr], verbose=0,
                           y=to_categorical(test_yy, num_classes=nc))


        # test_pred = mdl.predict(x=[test_rq, test_qr, test_lex_rq, test_lex_qr])
        test_pred = mdl.predict(x=[test_lex_rq, test_lex_qr])
